...
YEARS = as.character(2002:2014)
PREFIX = "IC"
SUFFIX = "_AY"
BASE_URL = "http://nces.ed.gov/ipeds/datacenter/data/%s%s%s.zip"

## loop and get the raw datasets
for (YEAR in YEARS) {
  ## build the URL
  URL = sprintf(BASE_URL, PREFIX, YEAR, SUFFIX)
  ## download the zip file
  download.file(URL, paste0(PREFIX, YEAR, SUFFIX,  ".zip"))
}
...
for (YEAR in YEARS) {
  ...
  ## bind
  charges = dplyr::bind_rows(charges, survey)
  ## cleanup
  rm(x, survey)
}
...
## save the data
save(charges, file="../data/charges_ay.rda")
